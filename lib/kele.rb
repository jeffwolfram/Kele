...
class Kele
  ...
  def create_submission(assignment_branch, assignment_commit_link, checkpoint_id, comment)
    self.class.post(base_url("checkpoint_submissions"), headers: {'authorization' => @auth_token}, body: { 'assignment_branch': assignment_branch, 'assignment_commit_link': assignment_commit_link, 'checkpoint_id': checkpoint_id, 'comment': comment, 'enrollment_id': get_me["current_enrollment"]["id"] } )
